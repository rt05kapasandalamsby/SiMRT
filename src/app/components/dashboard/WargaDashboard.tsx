...
import {
  Home, FileText, Bell, User, LogOut,
  Menu, X, Info, CreditCard, Calendar, ChevronRight,
  Download, Clock, CheckCircle2, MapPin, Phone, Loader2,
  ExternalLink, AlertCircle,
} from "lucide-react";
import {
  collection, query, where, onSnapshot, orderBy, updateDoc, doc, increment,
} from "firebase/firestore";
import { db } from "../../../firebase";
import { getSession, logoutUser, type AuthUser } from "../../../auth";
import { rtInfo, COLL, type PengumumanDoc, type IuranDoc, type SuratDoc, type TemplateDoc } from "./data";
...
type WargaPage = "beranda" | "pengumuman" | "iuran" | "janji" | "surat" | "profil";
...
// ─── Warga Beranda ────────────────────────────────────────────────────────────
function WargaBeranda({ user, setPage }: { user: AuthUser; setPage: (p: WargaPage) => void }) {
  const [pengumuman,   setPengumuman]   = useState<(PengumumanDoc & { id: string })[]>([]);
  const [iuranStatus,  setIuranStatus]  = useState<(IuranDoc & { id: string }) | null>(null);
  ...
  useEffect(() => {
    ...
    unsubs.push(onSnapshot(
      query(collection(db, COLL.pengumuman), where("status", "==", "Aktif"), orderBy("createdAt", "desc")),
      (snap) => {
        setPengumuman(snap.docs.map((d) => ({ id: d.id, ...d.data() } as PengumumanDoc & { id: string })));
        setLoading(false);
      },
      () => setLoading(false),
    ));
    ...
    unsubs.push(onSnapshot(
      ...
      (snap) => {
        if (!snap.empty) {
          setIuranStatus({ id: snap.docs[0].id, ...snap.docs[0].data() } as IuranDoc & { id: string });
        } else {
          setIuranStatus(null);
        }
      },
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-white font-bold text-lg leading-snug truncate">{user.name}</h1>
        ...
        <p className="text-blue-300 text-xs mt-3">
          RT {rtInfo.rt} / RW {rtInfo.rw} — Kel. {rtInfo.kelurahan}, Kec. {rtInfo.kecamatan}, {rtInfo.kota}
        </p>
      </motion.div>
      ...
      {iuranStatus && (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className={`font-semibold text-sm ${iuranStatus.status === "Lunas" ? "text-emerald-700" : "text-orange-700"}`}>
          ...
          <button onClick={() => setPage("iuran")} className="text-xs text-blue-600 hover:underline flex items-center gap-1 flex-shrink-0">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { label: "Pengumuman",   icon: Bell,       page: "pengumuman" as WargaPage, bg: "bg-orange-50  text-orange-600  hover:bg-orange-100",  count: pengumuman.length },
            { label: "Iuran Saya",   icon: CreditCard, page: "iuran"      as WargaPage, bg: "bg-blue-50    text-blue-600    hover:bg-blue-100",    count: null },
            { label: "Janji Temu",   icon: Calendar,   page: "janji"      as WargaPage, bg: "bg-violet-50  text-violet-600  hover:bg-violet-100",  count: null },
            { label: "Surat & Template", icon: FileText, page: "surat"   as WargaPage, bg: "bg-emerald-50 text-emerald-600 hover:bg-emerald-100", count: null },
            { label: "Profil Saya",  icon: User,       page: "profil"     as WargaPage, bg: "bg-slate-50   text-slate-600   hover:bg-slate-100",   count: null },
          ].map((item, i) => {
            ...
            {pengumuman.slice(0, 3).map((p, i) => (
              <AnimListItem key={p.id} index={i}>
                <div className="bg-white rounded-xl border border-slate-100 p-3.5 flex gap-3 hover:border-blue-200 transition-colors">
                  <div className={`w-1.5 self-stretch rounded-full flex-shrink-0 ${p.prioritas === "Tinggi" ? "bg-red-400" : "bg-blue-400"}`} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-800 truncate">{p.judul}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{p.tanggal} · {p.penulis}</p>
                  </div>
                  {p.prioritas === "Tinggi" && (
                    <span className="flex-shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-50 text-red-600 h-fit">Penting</span>
                  )}
                </div>
              </AnimListItem>
            ))}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-sm font-semibold text-blue-800">Kontak RT 05 Kapasan Dalam</p>
        </div>
        <div className="space-y-1 text-xs text-blue-700">
          <p className="flex items-center gap-1.5"><User className="w-3 h-3" /> Ketua RT: <strong>{rtInfo.ketua}</strong></p>
          <p className="flex items-center gap-1.5"><Phone className="w-3 h-3" /> {rtInfo.noHP}</p>
          <p className="flex items-center gap-1.5"><MapPin className="w-3 h-3" /> {rtInfo.alamatKantor}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-0.5">{aktif.length} pengumuman aktif</p>
      </div>
      {loading ? (
        ...
        <div className="space-y-3">
          {items.map((p, i) => (
            <AnimListItem key={p.id} index={i}>
              <div className={`bg-white rounded-2xl border p-4 ${p.status === "Aktif" ? "border-slate-100" : "border-dashed border-slate-200 opacity-60"}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ${p.prioritas === "Tinggi" ? "bg-red-400" : p.prioritas === "Normal" ? "bg-blue-400" : "bg-slate-300"}`} />
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      <h3 className="font-semibold text-slate-800 text-sm leading-snug">{p.judul}</h3>
                      <div className="flex gap-1.5 flex-shrink-0">
                        {p.prioritas === "Tinggi" && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-50 text-red-600">Penting</span>}
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${p.status === "Aktif" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>{p.status}</span>
                      </div>
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed mb-2">{p.isi}</p>
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                      <span>{p.tanggal}</span><span>•</span><span>{p.penulis}</span>
                    </div>
                  </div>
                </div>
              </div>
...
function WargaSurat({ user }: { user: AuthUser }) {
  ...
  const [templateList,  setTemplateList]  = useState<(TemplateDoc & { id: string })[]>([]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setSuratList(snap.docs.map((d) => ({ id: d.id, ...d.data() } as SuratDoc & { id: string })));
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      setTemplateList(snap.docs.map((d) => ({ id: d.id, ...d.data() } as TemplateDoc & { id: string })));
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-0.5">Permohonan surat & template dokumen RT 05</p>
      </div>
        ...
        <div className="space-y-3">
          {loadingSurat ? (
            ...
            <div className="p-10 text-center bg-white rounded-2xl border border-slate-100">
              ...
              <p className="text-slate-400 font-medium">Belum ada permohonan surat</p>
              ...
              <div className="mt-4 p-3 bg-blue-50 rounded-xl text-xs text-blue-600 text-left flex items-start gap-2">
                ...
                <span>Ketua RT: {rtInfo.ketua} · {rtInfo.noHP}</span>
              </div>
            </div>
          ) : (
              ...
              return (
                <AnimListItem key={item.id} index={i}>
                  <div className={cn(
                    ...
                    item.status === "Selesai" ? "border-emerald-200" :
                    item.status === "Menunggu" ? "border-orange-200" : "border-slate-100",
                  )}>
                    <div className="flex items-start gap-3">
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0",
                        item.status === "Selesai" ? "bg-emerald-100" :
                        item.status === "Diproses" ? "bg-blue-100" :
                        item.status === "Menunggu" ? "bg-orange-100" : "bg-red-100",
                      )}>
                        <StatusIcon className={cn("w-5 h-5",
                          item.status === "Selesai" ? "text-emerald-600" :
                          item.status === "Diproses" ? "text-blue-600" :
                          item.status === "Menunggu" ? "text-orange-500" : "text-red-500",
                        )} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="text-sm font-semibold text-slate-800">{item.jenis}</p>
                            <p className="text-xs text-slate-400 mt-0.5">{item.noSurat} · {item.tanggal}</p>
                          </div>
                          <span className={cn("text-xs font-semibold px-2.5 py-1 rounded-full flex-shrink-0", statusColor(item.status))}>
                            {item.status}
                          </span>
                        </div>
                        {item.catatan && (
                        ...
                        {item.status === "Selesai" && !item.downloadUrl && (
                          <div className="mt-2 flex items-center gap-1.5 text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2">
                            ...
                            File belum tersedia. Hubungi pengurus RT untuk mendapatkan surat.
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {tab === "template" && (
        <div className="space-y-3">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-800">Template Dokumen Resmi RT 05</p>
              <p className="text-xs text-blue-600 mt-0.5">
                Unduh template, isi data yang diperlukan, lalu bawa ke pengurus RT untuk ditandatangani.
                Hubungi <strong>{rtInfo.ketua}</strong> di {rtInfo.noHP} jika ada pertanyaan.
              </p>
            </div>
          </div>
          ...
          {loadingTempl ? (
            ...
            <div className="p-10 text-center bg-white rounded-2xl border border-slate-100">
              <FileText className="w-10 h-10 text-slate-200 mx-auto mb-3" />
              <p className="text-slate-400 font-medium">Template belum tersedia</p>
              <p className="text-xs text-slate-300 mt-1">
                Admin RT belum menambahkan template. Hubungi pengurus RT secara langsung.
              </p>
            </div>
          ) : (
                  ...
                  <div className="bg-white rounded-2xl border border-slate-100 p-4 flex items-start gap-3 hover:border-blue-200 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 text-xs text-slate-400">
                        ...
                        {t.ukuran && <><span>·</span><span>{t.ukuran}</span></>}
                      </div>
                      {t.deskripsi && (
                        <p className="text-xs text-slate-500 mt-1.5 line-clamp-2">{t.deskripsi}</p>
                      )}
                    </div>
                    <button
                      onClick={() => handleDownloadTemplate(t)}
                      disabled={downloading === t.id || !t.downloadUrl}
                      title={!t.downloadUrl ? "File belum tersedia" : "Unduh template"}
                      className={cn(
...
// ─── Warga Profil ─────────────────────────────────────────────────────────────
function WargaProfil({ user, onLogout }: { user: AuthUser; onLogout: () => void }) {
  ...
  const fields = [
    ...
    { label: "RT/RW",   value: `RT ${rtInfo.rt} / RW ${rtInfo.rw} — Kel. ${rtInfo.kelurahan}, ${rtInfo.kota}`, placeholder: "" },
  ];
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
        ...
        <div className="flex items-center gap-5 pb-5 border-b border-slate-100 mb-5">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-bold text-slate-800 text-lg leading-snug truncate">
              {name || <span className="text-slate-400 font-normal italic">Nama belum tersedia</span>}
            </h2>
            <span className="inline-flex items-center gap-1.5 bg-emerald-100 text-emerald-700 text-xs font-semibold px-2.5 py-1 rounded-full mt-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> Warga Aktif
            </span>
          </div>
        </div>
          ...
          <div className="mb-4 p-3.5 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-600 mt-0.5">
                Beberapa informasi Anda masih kosong. Hubungi pengurus RT untuk memperbarui data.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-0">
          {fields.map(({ label, value, placeholder }) => (
            <div key={label} className="flex gap-4 py-3 border-b border-slate-50 last:border-0">
              <span className="text-xs text-slate-400 w-20 flex-shrink-0 pt-0.5 font-medium">{label}</span>
              {value ? (
                <span className="text-sm text-slate-700 flex-1">{value}</span>
              ) : (
                <span className="text-sm text-slate-300 italic flex-1">{placeholder}</span>
              )}
            </div>
          ))}
        </div>
        ...
        {isIncomplete && (
          <div className="mt-5 p-4 bg-blue-50 border border-blue-100 rounded-xl text-center">
            <p className="text-sm text-blue-700 font-semibold mb-1">Lengkapi Profil Anda</p>
            <p className="text-xs text-blue-500">
              Hubungi pengurus RT 05 untuk melengkapi data nomor telepon dan alamat Anda.
            </p>
          </div>
        )}
...
export function WargaDashboard() {
  ...
  useEffect(() => {
    ...
    if (session.role !== "Warga") { navigate("/dashboard", { replace: true }); return; }
  ...
  const renderPage = () => {
    switch (activePage) {
      case "beranda":    return <WargaBeranda    user={currentUser} setPage={handleSetPage} />;
      case "pengumuman": return <WargaPengumuman />;
      case "iuran":      return <WargaIuran      user={currentUser} />;
      case "janji":      return <WargaJanjiTemu  user={currentUser} />;
      case "surat":      return <WargaSurat      user={currentUser} />;
      case "profil":     return <WargaProfil     user={currentUser} onLogout={handleLogout} />;
      default:           return <WargaBeranda    user={currentUser} setPage={handleSetPage} />;
    }
  };
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-[#0F2744] flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/10 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-blue-300 text-[11px] mt-0.5 leading-none">Kel. {rtInfo.kelurahan}, {rtInfo.kota}</div>
          </div>
        ...
        <div className="border-t border-white/10 px-3 py-3 flex-shrink-0 space-y-1">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-blue-300 text-[10px] mt-0.5">Warga RT {rtInfo.rt}</div>
            </div>
          </div>
          <button
        ...
        <header className="flex-shrink-0 h-16 bg-white border-b border-slate-200 flex items-center px-4 sm:px-6 gap-4 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:block">
              ...
              <div className="text-xs text-slate-400 mt-0.5">Warga RT {rtInfo.rt}</div>
            </div>
          </div>
        </header>
